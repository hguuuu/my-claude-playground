...
```
/                          research corpus (unchanged, constitution governs)
...
└── study/                 the study system (this design)
    ...
    ├── STATUS.md          dashboard: phase, active foci, metrics, next actions
...
### 8.4 Perspective boundaries — what Claude is and is not
...
2. Write `RULES.md` (language rules, templates, tag registry seed, frontmatter conventions, SQLite schema v1 + triggers, cadences) and `STATUS.md` + `skills.md` seeded from Phase 1 exit criteria — several likely already passed given the headstart; first weekly review tests them.
3. Create `journal/`, `cases/`, `examples/`, `quizzes/`, `reviews/weekly/`, `systems/*` with INDEX stubs + templates; `questions.md`, `comparative-table.md`, `错题本.md`.
4. Amend CLAUDE.md: `study:` prefix, ritual exemption, firewall pointers (Tier-2, this branch).
5. Re-point `slice1-train-001/002` deliverables into the study system (still doubling as time-module fixtures).
6. Wire the frontmatter validator stub into `formal/tests/run_tests.sh`.
7. Update `/queue/` and all touched INDEXes; session log; "ready for review."
